...
// Yes, this could be done in one pass (also the height check could be made more
// efficient, but as this is only called at game end it was not worth optimizing.
int Lyngk::StackCount(int player, int height) {
  int count = 0;
  for (int row = 0; row < NUM_ROWS; row++) {
    for (int col = 0; col < NUM_COLS; col++) {
      if (StackHeight(row, col) == height && StackOwner(row, col) == player) {
	count++;
      }
    }
  }
...
void Lyngk::PrintBoardJS(FILE *fp) {
  for (int col = 0; col < NUM_COLS; col++) {
    for (int row = 0; row < NUM_ROWS; row++) {
      if (OnBoard(row, col)) {
	int h = StackHeight(row, col);
	for (int i = h - 1; i >= 0; i--) {
	  fputc(GetAt(row, col * MAX_STACK + i), fp);
	}
	fputc('|', fp);
      }
    }
